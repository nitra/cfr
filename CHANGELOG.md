...
- Canonicalize KCC Pub/Sub topic and subscription IDs to their full Cloud
  Asset paths, eliminating false orphan/uncovered pairs.
- Resolve KCC `IAMPolicyMember` bucket references through the referenced
  `StorageBucket.spec.resourceID` when it differs from the Kubernetes name.
...
- Keep ownership exceptions out of the universal KCC inventory. Consumers
  should enrich its findings with their own repository-local policy.
