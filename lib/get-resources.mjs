...
async function listWorkloadIdentityPools(project) {
  return paginate(
    `${IAM_API}/projects/${project}/locations/global/workloadIdentityPools`,
    { pageSize: 1000 },
    'workloadIdentityPools',
  );
}

async function listWorkloadIdentityPoolProviders(poolName) {
  return paginate(`${IAM_API}/${poolName}/providers`, { pageSize: 1000 }, 'workloadIdentityPoolProviders');
}

function workloadIdentityPoolId(name) {
  const match = name && name.match(/\/locations\/([^/]+)\/workloadIdentityPools\/([^/]+)$/);
  return match && `${match[1]}/${match[2]}`;
}

function workloadIdentityPoolProviderId(name) {
  const match = name && name.match(/\/locations\/([^/]+)\/workloadIdentityPools\/([^/]+)\/providers\/([^/]+)$/);
  return match && `${match[1]}/${match[2]}/${match[3]}`;
}

export function isGkeWorkloadIdentityPool(pool) {
  return Boolean(pool.name && pool.name.endsWith('.svc.id.goog'));
}
...
export function liveIamPolicyResources(iamPolicies, project, includeSystem = false) {
  const resources = [];
...
export function liveIamPolicyIds(iamPolicies, project, includeSystem = false) {
  return liveIamPolicyResources(iamPolicies, project, includeSystem).map((resource) => resource.id);
}
...
export function kccBucketId(ref, bucketRefs) {
  return ref.external || bucketRefs.get(ref.name) || ref.name || '?';
}

export function canonicalPubSubId(id, project, collection) {
  if (!id || id.startsWith('projects/')) return id;
  return `projects/${project}/${collection}/${id}`;
}

function kccPolicyMemberId(item, project, runServiceRefs, bucketRefs) {
  ...
  switch (ref.kind) {
    ...
    case 'StorageBucket':
      return 'bucket/' + kccBucketId(ref, bucketRefs);
    case 'RunService': {
...
export async function collectNamespace(namespace, { includeSystem = false } = {}) {
  ...
  const resources = [];
  const diagnostics = [];
  const push = (kind, entries, source) => {
    for (const entry of entries) {
      if (typeof entry === 'string' && entry) resources.push({ kind, id: entry, source });
      else if (entry && entry.id) resources.push({ kind, ...entry, source });
    }
  };
  ...
  // --- IAMWorkloadIdentityPool + Provider ----------------------------------
  // Cloud Asset Inventory не гарантує повне покриття IAM federation
  // resources, тому читаємо їх напряму через IAM REST API.
  const allWorkloadIdentityPools = await listWorkloadIdentityPools(project);
  const gkeWorkloadIdentityPools = allWorkloadIdentityPools.filter(isGkeWorkloadIdentityPool);
  const workloadIdentityPools = includeSystem
    ? allWorkloadIdentityPools
    : allWorkloadIdentityPools.filter((pool) => !isGkeWorkloadIdentityPool(pool));
  if (!includeSystem && gkeWorkloadIdentityPools.length) {
    diagnostics.push({
      kind: 'IAMWorkloadIdentityPool',
      type: 'system-resource-skip',
      count: gkeWorkloadIdentityPools.length,
      reason: 'GKE-managed Workload Identity pool',
    });
  }
  push('IAMWorkloadIdentityPool', workloadIdentityPools.map((pool) => workloadIdentityPoolId(pool.name)), 'gcp');
  const kccWorkloadIdentityPools = await listCustomObjects('IAMWorkloadIdentityPool', namespace);
  const workloadIdentityPoolRefs = new Map(kccWorkloadIdentityPools.map((item) => [
    item.metadata && item.metadata.name,
    kccScopedId(item, { defaultLocation: 'global' }),
  ]));
  push('IAMWorkloadIdentityPool', [...workloadIdentityPoolRefs.values()], 'kcc');

  const workloadIdentityProviders = (await Promise.all(
    allWorkloadIdentityPools
      .filter((pool) => !isGkeWorkloadIdentityPool(pool))
      .map((pool) => listWorkloadIdentityPoolProviders(pool.name)),
  )).flat();
  push(
    'IAMWorkloadIdentityPoolProvider',
    workloadIdentityProviders.map((provider) => workloadIdentityPoolProviderId(provider.name)),
    'gcp',
  );
  push('IAMWorkloadIdentityPoolProvider', await kccField(
    'IAMWorkloadIdentityPoolProvider', namespace,
    (item) => {
      const ref = (item.spec && item.spec.workloadIdentityPoolRef) || {};
      const external = ref.external || '';
      const externalId = workloadIdentityPoolId(external);
      const poolId = externalId || workloadIdentityPoolRefs.get(ref.name);
      return poolId && `${poolId}/${resourceId(item)}`;
    },
  ), 'kcc');
  ...
  const storageBucketItems = await listCustomObjects('StorageBucket', namespace);
  push('StorageBucket', storageBucketItems.map((i) => resourceId(i)).filter(Boolean), 'kcc');
  ...
  push('PubSubTopic', await kccField(
    'PubSubTopic', namespace,
    (i) => canonicalPubSubId(resourceId(i), project, 'topics'),
  ), 'kcc');
  push('PubSubSubscription', byType('pubsub.googleapis.com/Subscription').map((a) => a.displayName), 'gcp');
  push('PubSubSubscription', await kccField(
    'PubSubSubscription', namespace,
    (i) => canonicalPubSubId(resourceId(i), project, 'subscriptions'),
  ), 'kcc');
  ...
  push('IAMPolicyMember', liveIamPolicyResources(iamPolicies, project, includeSystem), 'gcp');

  const runServiceRefs = new Map(runServiceItems.map((i) => [i.metadata && i.metadata.name, kccScopedId(i)]));
  const bucketRefs = new Map(storageBucketItems.map((i) => [i.metadata && i.metadata.name, resourceId(i)]));
  push('IAMPolicyMember', await kccField(
    'IAMPolicyMember', namespace,
    (item) => {
      const rid = kccPolicyMemberId(item, project, runServiceRefs, bucketRefs);
      const spec = item.spec || {};
      return {
        id: `${rid}/${spec.role}/${spec.member}`,
