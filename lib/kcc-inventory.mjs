...
Reports three states per kind:
  DRIFT             — live in GCP, not declared under a known controller
...
function report(kind, liveIds, kccIds, controlled, project, jsonMode, results) {
  const live = [...new Set(liveIds.filter(Boolean))].sort();
  const kcc = [...new Set(kccIds.filter(Boolean))].sort();
  const controlledIds = [...new Set(controlled.map(({ id }) => id).filter(Boolean))].sort();
  const kccSet = new Set(kcc);
  const liveSet = new Set(live);
  const controlledSet = new Set(controlledIds);
  const drift = live.filter((x) => !kccSet.has(x) && !controlledSet.has(x)).sort();
  const orphan = kcc.filter((x) => !liveSet.has(x)).sort();
  const covered = controlledIds.filter((x) => liveSet.has(x));
  const missingControlled = controlledIds.filter((x) => !liveSet.has(x));
  ...
  if (!jsonMode) {
    ...
    if (covered.length) {
      console.log('  COVERED_OPENTOFU — є в GCP і явно під OpenTofu:');
      for (const x of covered) console.log(`    ${x}`);
    }
    if (missingControlled.length) {
      console.log('  CONTROLLED_MISSING — заявлений під OpenTofu, але зник з GCP:');
      for (const x of missingControlled) console.log(`    ${x}`);
    }
    if (!drift.length && !orphan.length && !covered.length && !missingControlled.length) {
  ...
  for (const x of covered) results.push({ kind, id: x, project, status: 'covered_opentofu' });
  for (const x of missingControlled) results.push({ kind, id: x, project, status: 'controlled_missing_opentofu' });
}
...
// per-kind {live, kcc, controlled} arrays, diffs and prints each kind in KIND_ORDER,
...
function diffAndReport(collected, jsonMode, results) {
  ...
  const byKind = new Map(KIND_ORDER.map((k) => [k, { live: [], kcc: [], controlled: [] }]));
  for (const { kind, id, source } of resources) {
    const bucket = byKind.get(kind);
    if (!bucket) continue;
    if (source === 'kcc') bucket.kcc.push(id);
    else if (source === 'gcp') bucket.live.push(id);
    else bucket.controlled.push({ id, source });
  }
  ...
  for (const kind of KIND_ORDER) {
    ...
    const { live, kcc, controlled } = byKind.get(kind);
    report(kind, live, kcc, controlled, project, jsonMode, results);
  }
}
