...
export function printDiagnostic(diag, jsonMode, results, project) {
  if (diag.type === 'stale-cache') {
    ...
    if (jsonMode) {
      results.push({ kind: diag.kind, id: `gke_managed:${diag.zones.length}`, project, status: 'ignored' });
    } else {
  ...
  for (const kind of KIND_ORDER) {
    ...
    for (const entry of visible.sort((a, b) => a.id.localeCompare(b.id))) {
      ...
      const condition = entry.condition
        ...
        : '';
      console.log(`  ${entry.status.toUpperCase()}${owner} — ${entry.id}${condition}`);
    }
    console.log('');
  }
